...
export default class RegExpUtils {
  ...
  // 验证手机机身码(IMEI)
  isIMEI = (value) => /^\d{15,17}$/g.test(value)
  ...
  // 验证迅雷链接
  isThunderLink = (value) => /^thunderx?:\/\/[a-zA-Z\d]+=$/g.test(value)

  // 验证ed2k链接(宽松匹配)
  ised2k = (value) => /^ed2k:\/\/\|file\|.+\|\/$/g.test(value)

  // 验证磁力链接(宽松匹配)
  isMagnet = (value) => /^magnet:\?xt=urn:btih:[0-9a-fA-F]{40,}.*$/g.test(value)

  // 验证子网掩码
  isSubnetMask = (value) => /^(?:\d{1,2}|1\d\d|2[0-4]\d|25[0-5])(?:\.(?:\d{1,2}|1\d\d|2[0-4]\d|25[0-5])){3}$/g.test(value)

  // 验证linux"文件夹"路径
  isLinuxFolderPath = (value) => /^(\/[^/]+)+\/?$/g.test(value)

  // 验证linux"文件"路径
  isLinuxFilePath = (value) => /^(\/[^/]+)+$/g.test(value)

  // 验证window"文件夹"路径
  isWindowsFolderPath = (value) => /^[a-zA-Z]:\\(?:\w+\\?)*$/g.test(value)

  // 验证window下"文件"路径
  isWindowsFilePath = (value) => /^[a-zA-Z]:\\(?:\w+\\)*\w+\.\w+$/g.test(value)

  // 验证股票代码(A股)
  isAShare = (value) => /^(s[hz]|S[HZ])(000[\d]{3}|002[\d]{3}|300[\d]{3}|600[\d]{3}|60[\d]{4})$/g.test(value)

  // 验证版本号格式必须为X.Y.Z
  isVersion = (value) => /^\d+(?:\.\d+){2}$/g.test(value)

  // 验证视频链接地址（视频格式可按需增删）
  isVideoUrl = (value) => /^https?:\/\/(.+\/)+.+(\.(swf|avi|flv|mpg|rm|mov|wav|asf|3gp|mkv|rmvb|mp4))$/i.test(value)

  // 验证图片链接地址（图片格式可按需增删）
  isImageUrl = (value) => /^https?:\/\/(.+\/)+.+(\.(gif|png|jpg|jpeg|webp|svg|psd|bmp|tif))$/i.test(value)

  // 验证银行卡号（10到30位, 覆盖对公/私账户, 参考微信支付）
  isAccountNumber = (value) => /^[1-9]\d{9,29}$/g.test(value)
  ...
  // 验证身份证号(1代,15位数字)
  isIDCardOld = (value) => /^\d{8}(0\d|10|11|12)([0-2]\d|30|31)\d{3}$/g.test(value)

  // 验证身份证号(2代,18位数字),最后一位是校验位,可能为数字或字符X
  isIDCardNew = (value) => /^\d{6}(18|19|20)\d{2}(0\d|10|11|12)([0-2]\d|30|31)\d{3}[\dXx]$/g.test(value)

  // 验证身份证号, 支持1/2代(15位/18位数字)
  isIDCard = (value) => /(^\d{8}(0\d|10|11|12)([0-2]\d|30|31)\d{3}$)|(^\d{6}(18|19|20)\d{2}(0\d|10|11|12)([0-2]\d|30|31)\d{3}(\d|X|x)$)/g.test(value)
  ...
  isWebAccount = (value) => /^[a-zA-Z]\w{4,15}$/g.test(value)
  ...
  // 验证数字
  isNumber = (value) => /^\d{1,}$/g.test(value)
